...
#Usage $python parse_sifts.py 1gag.xml.gz

import os, sys, gzip, subprocess
import xml.etree.ElementTree as ET

def parse_sifts(filename):
        ...
        fhandle=open(filename[0:-7].lower()+".csv","w")
                            ...
                            if listResidue.tag==(str(base)+'listResidue'):
                                for residue in listResidue:
                                    ...
                                    if 'Insertion' in pdbe_property and prev_uninum_insertion!=-9999:        #check for insertion and make sure it is not in the beginning of the chain by -9999
                                        ...
                                        insertion_code='A'
        fhandle.close()
        cmd=("gzip -f "+filename[0:-7].lower()+".csv")
        subprocess.call(cmd, shell=True)


if __name__=='__main__':
    filename=sys.argv[1]
    parse_sifts(filename)
